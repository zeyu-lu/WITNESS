...
public class AssertionCounterListener extends JavaParserBaseListener {
    ...
    public AssertionCounterListener(Parser parser, BufferedTokenStream tokens, String workdir, String srcTests, String testClass, List<String> testList, Map<String, Integer> testMap, MultiMap<String, String> paramMap){
        this.parser = parser;
        this.tokens = tokens;
    ...
    public void setAssertNum(int assertNum) {
        ...
        if (ctx.memberDeclaration() != null){
            if (ctx.memberDeclaration().methodDeclaration() != null) {
                ...
                if (testList.contains(testName) ) {
                    ...
                    for (ParseTree t : XPath.findAll(ctx, "//blockStatement/statement/expression/methodCall/*", parser)){
                        if (t instanceof JavaParser.IdentifierContext){
                            JavaParser.IdentifierContext identifierContext = (JavaParser.IdentifierContext) t;
                            String methodName = identifierContext.getText();
                            if (methodName.contains("assert") || methodName.equals("fail") || methodName.equals("test")) {
                                assertionCount++;
                            }
                        }
                    }
                    setAssertNum(getAssertNum() + assertionCount);
                    //
                } else {
