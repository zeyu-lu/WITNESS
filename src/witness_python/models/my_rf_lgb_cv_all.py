...
test_dataset_path = test_data_dir + 'feature_data.txt'
...
inside_method_predictions = output_test_df.merge(covmap_methods, on=['MutantNo', 'TestNo'], how='inner')

# Find rows in csv1 not matching covmap_methods (outside_method_predictions)
outside_method_predictions = output_test_df.merge(covmap_methods, on=['MutantNo', 'TestNo'], how='left', indicator=True)
